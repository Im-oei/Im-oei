...
async function initLiff() {
  try {
    ...
    // Auto-login เฉพาะเมื่อ redirect กลับมาจาก LINE login (มี flag ใน localStorage)
    ...
    } else if (liff.isLoggedIn() && !cameFromLine) {
      // iOS กลับมาจาก LINE แต่ flag หาย — ลอง login อีกครั้ง
      await handleLiffLogin();
    }
  } catch (e) {
    console.warn('LIFF init error:', e.message);
    // ถ้า error เกี่ยวกับ unauthorized redirect URI — แสดงข้อความช่วยเหลือ
    if (e.message && (e.message.includes('unauthorize') || e.message.includes('redirect'))) {
      showError('กรุณาเพิ่ม URL นี้ใน LINE Developers Console:\n' + window.location.origin + '/login.html');
    }
  }
}
...
window._mod_lineLogin = window.lineLogin = async function() {
  try {
    ...
    // Init LIFF ถ้ายังไม่ได้ init — ใช้ withLoginOnExternalBrowser เสมอ
    if (typeof liff.isInitialized === 'function' ? !liff.isInitialized() : !window._liffInited) {
      try {
        await liff.init({ liffId: LIFF_ID, withLoginOnExternalBrowser: true });
        window._liffInited = true;
      } catch(initErr) {
        console.warn('LIFF re-init error:', initErr.message);
      }
    }
    ...
    if (liff.isLoggedIn()) {
      ...
      // บน iOS: ใช้ redirectUri แบบ absolute path เพื่อป้องกัน cross-origin issue
...
// LIFF SDK ถูก load จาก login.html — รอให้พร้อมก่อน init
function waitForLiffAndInit(retries) {
  if (typeof liff !== 'undefined') {
    initLiff();
  } else if (retries > 0) {
    setTimeout(() => waitForLiffAndInit(retries - 1), 150);
  } else {
    console.warn('LIFF SDK not available after waiting');
  }
}
waitForLiffAndInit(20);
