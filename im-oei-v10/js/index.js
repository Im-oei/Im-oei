...
function getUser(){
  try {
    ...
    if (u && u.loginAt && u.role!=='admin' && u.role!=='owner' && (Date.now() - u.loginAt > 24*60*60*1000)) {
...
function buildFeatured(){
  ...
  // ไม่แสดง dots — วนอัตโนมัติ
  wrap.innerHTML = '<div class="fc-track" id="fc-track">'+cards+'</div>';

  if (items.length <= 1) return;

  var track   = document.getElementById('fc-track');
  var dotsEl = null; // dots ถูกลบแล้ว
  _featIdx = 0;
