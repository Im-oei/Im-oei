...
if(user.loginAt && user.role!=='admin' && user.role!=='owner' && (Date.now() - user.loginAt > 24*60*60*1000)){
...
// ─── ดึงเบอร์ที่เคยบันทึกไว้กลับมา (ไม่ต้องใส่ซ้ำ) ──────────────────────
(function restoreSavedPhone() {
  try {
    ...
    // บันทึกเบอร์ถาวรแยกต่างหาก (คงอยู่แม้ session หมดอายุ)
    if (user.phone) {
      localStorage.setItem('imkum_saved_phone', user.phone);
    }
  } catch(e) {}
})();
...
function injectPhoneInputModal() {
  if (document.getElementById('orders-phone-overlay')) return;
  const modal = document.createElement('div');
  modal.id = 'orders-phone-overlay';
  ...
  modal.innerHTML = `
    <div style="width:100%;max-width:480px;background:#fff;border-radius:28px 28px 0 0;
      padding:28px 24px 40px;animation:slideUpPhone 0.3s cubic-bezier(0.34,1.56,0.64,1)">
      <style>@keyframes slideUpPhone{from{transform:translateY(100%)}to{transform:translateY(0)}}</style>
      <div style="font-family:'Prompt',sans-serif;font-size:20px;font-weight:800;color:#3E2000;text-align:center;margin-bottom:6px">
        📱 กรอกเบอร์โทรศัพท์
      </div>
      <div style="font-family:'Sarabun',sans-serif;font-size:13px;color:#888;text-align:center;margin-bottom:20px">
        เพื่อติดตามออเดอร์ของคุณ
      </div>
      <div style="display:flex;align-items:center;gap:10px;border:2px solid #E8E0D8;border-radius:14px;padding:4px 14px;margin-bottom:14px;background:#FAFAF8">
        <span style="font-family:'Sarabun',sans-serif;font-size:15px;font-weight:700;color:#888">📞</span>
      ...
      <div id="oorders-error" style="font-family:'Sarabun',sans-serif;font-size:13px;font-weight:700;color:#E53935;text-align:center;margin-bottom:10px;min-height:20px"></div>
      <button onclick="ordersPhoneConfirm()"
      ...
      <button onclick="ordersPhoneClose()"
...
window.openPhoneVerifyModal = function() {
  injectPhoneInputModal();
  document.getElementById('orders-phone-overlay').style.display = 'flex';
  document.getElementById('oorders-error').textContent = '';
  setTimeout(() => document.getElementById('oorders-phone')?.focus(), 100);
};
window.ordersPhoneClose = function() {
  document.getElementById('orders-phone-overlay').style.display = 'none';
};
window.ordersPhoneConfirm = async function() {
  const phone = (document.getElementById('oorders-phone').value || '').trim();
  if (!/^0[6-9]\d{8}$/.test(phone)) {
    document.getElementById('oorders-error').textContent = 'กรุณากรอกเบอร์ให้ถูกต้อง (ขึ้นต้นด้วย 06-09)';
    return;
  }
  document.getElementById('oorders-error').textContent = '';
  // อัปเดต session
  ...
  if (user.lineUserId) {
    try {
      await setDoc(doc(db, 'lineUsers', user.lineUserId), {
        userId: user.lineUserId,
        displayName: user.name || '',
        phone,
        updatedAt: serverTimestamp(),
      }, { merge: true });
      await setDoc(doc(db, 'linePhoneMap', phone), { userId: user.lineUserId, updatedAt: serverTimestamp() });
    } catch(e) { console.warn('lineUsers update:', e.message); }
  }
  // ซ่อน banner + modal แล้ว reload orders
  const banner = document.getElementById('phone-verify-banner');
  if (banner) banner.remove();
  window.ordersPhoneClose();
...
function renderStampCard(points, lifetimePoints, tierCfg) {
  ...
  let dots = '';
  const sandwichSVG = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 3l2.09 6.26L20 9.27l-4.91 3.73L16.18 19 12 15.77 7.82 19l1.09-5.99L4 9.27l5.91-.01L12 3z" fill="#FFD700" stroke="#FFA500" stroke-width="0.5"/></svg>`;
  const emptySVG = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="5" fill="rgba(255,255,255,0.2)"/></svg>`;
...
async function loadStamps(){
  ...
  const stampDocId = user.lineUserId || user.phone; // ต้องตรงกับ cart.module.js ที่บันทึก lineUserId || phone
  try{
